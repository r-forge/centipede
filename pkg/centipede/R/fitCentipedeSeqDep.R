...
##' @return ...
fitCentipedeSeqDep <- function(Xlist,Y, KmerSeq,Mappability, KmerCutPr, BetaLogit, LambdaParList, NegBinParList,sweeps=50,DampLambda=0.01,DampNegBin=0.0,TrimP=0.0001,NRiter=5,FixLambda=FALSE) {
  ...
  compLambdaNoMixSeqDep <- function(LambdaIni,X,R,DampLambda=0.0,rho){
    S <- dim(X)[2];
    if(S>1){
      ##    Ez <- Ez*(1-DampLambda)+0.5*(DampLambda); ## Dampening experiment
      ##Xs <- t(t(Ez) %*% X);
      ...
      LogParIni <- log(LambdaIni)
      optPar <- optim(LogParIni,fMultSeqDep,gMultSeqDep,
                      Xs = (as.matrix(colSums(X))),EzRl = R,rho = rho,
                      method="BFGS",control = list(maxit = NRiter))
      LogPar <- optPar$par
      Lambda <- exp(LogPar);
      ##sum(Lambda)
...
################################################################  
  ...
  ##Ez <- pnorm(qqnorm(Rlist[[1]])$x)
  Ez[Ez==0] <- runif(sum(Ez==0))/100
  ...
  if(missing(LambdaParList)){
    if(!FixLambda){
      ...
      LambdaParList <- lapply(Xlist,function(X){as.matrix(runif(dim(X)[2])/1000+0.00001)});
    }
  }
...
## NegBin initialization...   
  ...
  if(SeqDepFlag)
    LambdaParList <- mapply(compLambdaNoMixSeqDep, LambdaParList, Xlist,Rlist,MoreArgs=list(DampLambda=DampLambda,rho=RhoMat),SIMPLIFY=FALSE);
  ...
  if(SeqDepFlag)
    LogLikNoMix <- sum(mapply(compLogLik1SeqDep,Xlist,LambdaParNoMixList,NegBinParNoMixList,MoreArgs=list(TrimP=TrimP,rho=RhoMat,DampNegBin=DampNegBin)))
